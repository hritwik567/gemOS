...
{
    //lock
    dprintf("Hritvik %s %lx\n",__func__,cache_bmap[0]);
...
//----------------------------DELETE THIS-----------------------------------------
// u32 customhash(struct objfs_state *objfs ,char *key)
// {   //should never return 0 because when we store inode->hash_idx mapping
//     //hasidx = 0 means that inode is not allocated, we keep this as an identifier
//     //has size = HASH_SIZE
//     u32 k = 0;
...
//----------------------------DELETE THIS-----------------------------------------
// int custom_free_inode(struct objfs_state *objfs)
// {
//     dprintf("Hritvik: free_inode\n");
//     //implement a lock here
//     for(int i=0;i<1;i++){
//         for(int j=63;j>=0;j--){
//             //dprintf("Hritvik: free_inode(i,j,inode_bmap[i]) = %d, %d, %d, %d\n", i, j, inode_bmap[i], (inode_bmap[i]>>j)&1);
//             if((u64)((inode_bmap[i]>>j)&1) != 1){
//                 dprintf("%lx %lx\n",((u64)1<<63), inode_bmap[i]);
//                 inode_bmap[i] = inode_bmap[i] | ((u64)1<<j);
//                 dprintf("here in loop i, j, bmap %d %d %lx\n",i,j,inode_bmap[i]);
//                 return 64*i + 64 - j + 2;
//             }
//         }
//     }
...
//----------------------------DELETE THIS-----------------------------------------
// int custom_free_block(struct objfs_state *objfs)
// {
//     dprintf("Hritvik: free_block\n");
//     //implement a lock here
//     for(int i=0;i<(MAX_BLOCKS>>6);i++){
...
//             if((u64)((block_bmap[i]>>j)&1) != 1){
//                 block_bmap[i] = block_bmap[i] | ((u64)1<<j);
//                 dprintf("Hritvik: free_block(i,j,inode_bmap[i], metadata) = %d, %d, %lx, %d\n", i, j, block_bmap[i], metadata_blocks);
//                 return 64*i + 64 - j + metadata_blocks;
//             }
//         }
//     }
...
long destroy_object(const char *key, struct objfs_state *objfs)
{
    ...
    if(prev==NULL)dprintf("Hritvik: Yes prev is null");
    if(hashTable[hs]==NULL)dprintf("Hritvik: Yes hashTable is null");
    u32 blks = obj->blocks;
    u32 idx = 0;
    while(idx<L1_BLOCKS && blks>0) {
        dprintf("Hritvik: destroy_object l1_free %d metadatablks %d\n", obj->l1[idx], metadata_blocks);
...
long objstore_write(int objid, const char *buf, int size, struct objfs_state *objfs, off_t offset)
{
    ...
    while(size>0&&idx<L2_BLOCKS) {
        ...
        if(obj->l2[idx]==0){
            ...
            obj->l2[idx] = temp;
            if(find_write_cached(objfs, obj->l2[idx], b2r, BLOCK_SIZE)<0){
                free(obj);
                return -1;
            }
        } else {
            if(find_read_cached(objfs, obj->l2[idx], b2r, BLOCK_SIZE)<0){
                free(obj);
                return -1;
            }
            for(int i=0;i<l2_level_id;i++){
                dprintf("B2R ---> %d ----> %d\n",i,b2r[i]);
            }
        }
