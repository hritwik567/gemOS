...
static void schedule_context_exit(struct exec_context *next)
{
    /*Your code goes in here. get_current_ctx() still returns the old context*/
    struct exec_context *current = get_current_ctx();
    u32 free_page = current->os_stack_pfn;
    printf("scheduling: old pid = %d  new pid  = %d\n", current->pid, next->pid); /*XXX: Don't remove*/
    /*These two lines must be executed*/
    set_tss_stack_ptr(next);
    set_current_ctx(next);
    ...
    u64* rbp = omap(next->os_stack_pfn);
    
    *(rbp) = next->regs.entry_ss;
    *(rbp - 1) = next->regs.entry_rsp;
    *(rbp - 2) = next->regs.entry_rflags;
    *(rbp - 3) = next->regs.entry_cs;
    *(rbp - 4) = next->regs.entry_rip;
    ...
    *(rbp - 11) = next->regs.rbp;
    *(rbp - 12) = next->regs.r8;
    *(rbp - 13) = next->regs.r9;
    *(rbp - 14) = next->regs.r10;
    *(rbp - 15) = next->regs.r11;
    *(rbp - 16) = next->regs.r12;
    *(rbp - 17) = next->regs.r13;
    *(rbp - 18) = next->regs.r14;
    *(rbp - 19) = next->regs.r15;
    *(rbp - 20) = free_page;
...
long do_clone(void *th_func, void *user_stack)
{
    ...
    child->os_rsp = omap(child->os_stack_pfn);
...
{
    struct exec_context *current = get_current_ctx();
    u64 oldhandler = current->sighandlers[signo];
